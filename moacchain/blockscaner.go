...
//MOACBlockScanner ontology的区块链扫描器
type MOACBlockScanner struct {
...
//NewMOACBlockScanner 创建区块链扫描器
func NewMOACBlockScanner(wm *WalletManager) *MOACBlockScanner {
	bs := MOACBlockScanner{
...
func (bs *MOACBlockScanner) SetRescanBlockHeight(height uint64) error {
	height = height - 1
	if height < 0 {
...
//ScanBlockTask 扫描任务
func (bs *MOACBlockScanner) ScanBlockTask() {
...
//ScanBlock 扫描指定高度区块
func (bs *MOACBlockScanner) ScanBlock(height uint64) error {
...
func (bs *MOACBlockScanner) scanBlock(height uint64) (*Block,error) {
...
func (bs *MOACBlockScanner) ScanTxMemPool() {
...
func (bs *MOACBlockScanner) RescanFailedRecord() {
...
func (bs *MOACBlockScanner) newBlockNotify(block *Block, isFork bool) {
...
func (bs *MOACBlockScanner) BatchExtractTransaction(blockHeight uint64, blockHash string, txs []string, memPool bool) error {
...
func (bs *MOACBlockScanner) extractRuntime(producer chan ExtractResult, worker chan ExtractResult, quit chan struct{}) {
...
func (bs *MOACBlockScanner) ExtractTransaction(blockHeight uint64, blockHash string, txid string, scanAddressFunc openwallet.BlockScanAddressFunc, memPool bool) ExtractResult {
...
func convertToAmount(amount *big.Int,decimals int32) string {
...
// amount 字符串转为最小单位的表示
func convertFromAmount(amountStr string,decimals int32) *big.Int {
	...
	amount,_:=big.NewInt(0).SetString(d.String(),10)
	return amount
}

//ExtractTransactionData 提取交易单
func (bs *MOACBlockScanner) extractTransaction(trx *Transaction, result *ExtractResult, scanAddressFunc openwallet.BlockScanAddressFunc) {
	var (
		success = true
	)
	...
	if trx == nil {
		...
		if success && !trx.IsCoinBase {
			...
			if ok{
				...
				feeCharge.Amount = convertToAmount(trx.Fee,bs.wm.Decimal())
				...
				if ok{
					...
					output.Amount = convertToAmount(trx.Amount,bs.wm.Decimal())
			...
			for _, extractData := range result.extractData {
				...
				tx := &openwallet.Transaction{
					From:   []string{trx.From + ":" + convertToAmount(trx.Amount,bs.wm.Decimal())},
					To:     []string{trx.To + ":" + convertToAmount(trx.Amount,bs.wm.Decimal())},
					Amount: convertToAmount(trx.Amount,bs.wm.Decimal()),
					Fees:   convertToAmount(trx.Fee,bs.wm.Decimal()),
...
func (bs *MOACBlockScanner) newExtractDataNotify(height uint64, extractData map[string]*openwallet.TxExtractData) error {
...
//func (bs *MOACBlockScanner) SaveRechargeToWalletDB(height uint64, list []*openwallet.Recharge) error {
//
//	for _, r := range list {
//
...
func (bs *MOACBlockScanner) GetCurrentBlockHeader() (*openwallet.BlockHeader, error) {
	var (
...
func (bs *MOACBlockScanner) GetScannedBlockHeader() (*openwallet.BlockHeader, error) {
	...
	if blockHeight == 0 {
		blockHeight, err = bs.wm.GetBlockHeight()
		if err != nil {
			bs.wm.Log.Errorf("MOAC GetBlockHeight failed,err = %v", err)
			return nil, err
		}
...
func (bs *MOACBlockScanner) GetScannedBlockHeight() uint64 {
	localHeight, _, _ := bs.wm.GetLocalNewBlock()
	return localHeight
}

func (bs *MOACBlockScanner) ExtractTransactionData(txid string, scanTargetFunc openwallet.BlockScanTargetFunc) (map[string][]*openwallet.TxExtractData, error) {
...
//func (bs *MOACBlockScanner) DropRechargeRecords(accountID string) error {
//	bs.mu.RLock()
//	defer bs.mu.RUnlock()
//
...
//func (bs *MOACBlockScanner) DeleteRechargesByHeight(height uint64) error {
//
//	bs.mu.RLock()
//	defer bs.mu.RUnlock()
//
...
func (bs *MOACBlockScanner) SaveUnscanRecord(record *UnscanRecord) error {
...
func (bs *MOACBlockScanner) GetSourceKeyByAddress(address string) (string, bool) {
	bs.Mu.RLock()
	defer bs.Mu.RUnlock()
...
// func (bs *MOACBlockScanner) GetWalletByAddress(address string) (*openwallet.Wallet, bool) {
// 	bs.mu.RLock()
// 	defer bs.mu.RUnlock()
...
func (bs *MOACBlockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {
...
func (bs *MOACBlockScanner) GetTransactionsByAddress(offset, limit int, coin openwallet.Coin, address ...string) ([]*openwallet.TxExtractData, error) {
...
//Run 运行
func (bs *MOACBlockScanner) Run() error {
...
////Stop 停止扫描
func (bs *MOACBlockScanner) Stop() error {
...
//Pause 暂停扫描
func (bs *MOACBlockScanner) Pause() error {
...
//Restart 继续扫描
func (bs *MOACBlockScanner) Restart() error {
...
func (bs *MOACBlockScanner) setupSocketIO() error {
