...
func (decoder *TransactionDecoder) CreateRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.CreateMOACRawTransaction(wrapper, rawTx)
}

//SignRawTransaction 签名交易单
func (decoder *TransactionDecoder) SignRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.SignMOACRawTransaction(wrapper, rawTx)
}

//VerifyRawTransaction 验证交易单，验证交易单并返回加入签名后的交易单
func (decoder *TransactionDecoder) VerifyRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	return decoder.VerifyMOACRawTransaction(wrapper, rawTx)
}
...
func (decoder *TransactionDecoder) CreateMOACRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if len(rawTx.FeeRate) > 0 {
		gasPrice = convertFromAmount(rawTx.FeeRate, decoder.wm.Decimal())
	} else {
		gasPrice, err = decoder.wm.Client.getGasPrice()
		if err != nil {
			return err
		}
	}
	gasLimit, _ := big.NewInt(0).SetString(decoder.wm.Config.GasLimit, 10)

	fee := big.NewInt(0)
	fee = fee.Mul(gasPrice, gasLimit)
	...
	amount := convertFromAmount(amountStr, decoder.wm.Decimal())
	...
	for _, a := range addressesBalanceList {
		if a.Balance.Cmp(amount) < 0 {
			...
			if count.Cmp(amount) >= 0 {
				...
				decoder.wm.Log.Std.Notice("The MOAC of the account is enough," +
	...
	gasUsed, err := decoder.wm.Client.getGasEstimated(from, to, gasLimit, gasPrice, convertFromAmount(amountStr, decoder.wm.Decimal()))
	if err != nil {
		return err
	}
	fee = fee.Mul(gasUsed, gasPrice)
	rawTx.TxFrom = []string{from}
	rawTx.TxTo = []string{to}
	rawTx.TxAmount = amountStr
	rawTx.Fees = convertToAmount(fee, decoder.wm.Decimal())
	rawTx.FeeRate = convertToAmount(gasPrice, decoder.wm.Decimal())
	...
	emptyTrans, hash, err := moacchainTransaction.CreateEmptyRawTransactionAndHash(to, nonce, convertFromAmount(amountStr, decoder.wm.Decimal()), gasLimit, gasPrice, decoder.wm.Config.IsTestNet)
	if err != nil {
		return err
	}
...
func (decoder *TransactionDecoder) SignMOACRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			//签名交易
			///////交易单哈希签名
			signature, err := moacchainTransaction.SignRawTransaction(keySignature.Message, keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyMOACRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	pass, signedTrans := moacchainTransaction.VerifyAndCombineRawTransaction(emptyTrans, signature, pubkey, decoder.wm.Config.IsTestNet)
...
func (decoder *TransactionDecoder) GetRawTransactionFeeRate() (feeRate string, unit string, err error) {
	...
	return convertToAmount(gasPrice, decoder.wm.Decimal()), "Gas", nil
}
...
func (decoder *TransactionDecoder) CreateSimpleSummaryRawTransaction(wrapper openwallet.WalletDAI, sumRawTx *openwallet.SummaryRawTransaction) ([]*openwallet.RawTransaction, error) {
	...
	for _, addrBalance := range addrBalanceArray {
		...
		if len(sumRawTx.FeeRate) > 0 {
			gasPrice = convertFromAmount(sumRawTx.FeeRate, decoder.wm.Decimal())
		} else {
			gasPrice, err = decoder.wm.Client.getGasPrice()
			if err != nil {
				return nil, err
		...
		gasLimit, _ := big.NewInt(0).SetString(decoder.wm.Config.GasLimit, 10)
		//	gasUsed, _ := decoder.wm.Client.getGasEstimated(addrBalance.Address, sumRawTx.SummaryAddress, gasLimit, gasPrice, nil)
		...
		sumAmount := convertToAmount(sumAmount_BI, decoder.wm.Decimal())
		fees := convertToAmount(fee, decoder.wm.Decimal())
...
func (decoder *TransactionDecoder) createRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction, addrBalance *openwallet.Balance) error {
	...
	//	gasUsed, _ := decoder.wm.Client.getGasEstimated(addrBalance.Address, to, gasLimit, gasPrice, nil)
	fee := big.NewInt(0)
	fee = fee.Mul(gasPrice, gasLimit)
	amount := convertFromAmount(amountStr, decoder.wm.Decimal())
	...
	emptyTrans, hash, err := moacchainTransaction.CreateEmptyRawTransactionAndHash(to, nonce, amount, gasLimit, gasPrice, decoder.wm.Config.IsTestNet)
